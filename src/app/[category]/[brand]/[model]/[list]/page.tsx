...
import { CartItem, CurrentProduct, Equivalent, Producto } from "@/data";
	...
	useEffect(() => {
		const fetchProductos = async () => {
			try {
				...
				console.log(
					data,
					"esto es la respuesta de productos relacionados desde el back"
				);
			} catch (error) {
	...
	const handleAddToCart = useCallback(
		(product: Producto) => {
			const item: CartItem = {
				...
				quantity: quantity,
				image: product.image,
				imp_sales: "0", // o la lógica que definas tú
				imp_sales_code: "",
			};
			...
			<div className="max-w-5xl mx-auto bg-white shadow-md rounded-md p-1 m-5 bg-blue">
				...
				<div className="flex flex-col md:flex-row gap-6 rounded-lg p-6">
							...
							<div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
								<div className="relative bg-white p-4 rounded-lg shadow-lg">
									...
									<Image
										src={
											producto?.item?.image
												? `https://autofrioimportaciones.com/images/productos/${producto?.item?.image}`
												: "/placeholder.jpg"
										}
										alt="Imagen ampliada"
										width={500}
					...
					<div className="w-full md:w-3/5">
						<p className="text-primary font-regular text-sm">
						...
						{isAuthenticated ? (
							<div>
								<p className="mt-10 text-2xl font-semibold text-primary">
									{formatCurrency(producto?.item?.price)}
								</p>
						...
						<div className="flex flex-wrap gap-2 mt-2">
							{producto?.item?.equivalents.map((brand, i) => (
								<div
									key={i}
									className=" cursor-pointer hover:scale-105 hover:shadow-lg transition  flex items-center text-md px-4 py-1 rounded-sm shadow-md min-h-10"
								>
									<Image
										src={
											brand.logo
												? `https://autofrioimportaciones.com/images/productos/${brand.logo}`
												: "/placeholder.jpg"
										}
										alt={brand.codigo}
										width={82}
									...
									/>
									<span className="text-primary text-[10px] ml-[10px]">
										{isAuthenticated ? brand.price : brand.codigo}
									</span>
								</div>
							))}
						</div>
						...
						<div className="mt-4 flex flex-col gap-2">
							...
							{isAuthenticated ? (
								<button
									onClick={() => handleAddToCart(producto?.item)}
							...
							<div className="flex gap-2">
								{isAuthenticated ? (
									<button
										onClick={() => handleAddToCart(producto?.item)}
										className="border border-primary text-primary py-2 rounded-full font-bold hover:bg-primary transition w-1/2 text-[12px] py-3  hover:text-white"
									>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-1 p-6">
					<div>
						...
						<ul className="list-disc list-inside text-gray-400 mt-2 text-xs columns-2">
							{producto?.oem
								?.slice(0, showMoreApp ? producto?.oem?.length : 30)
								.map(
									(
										oem:
											| string
